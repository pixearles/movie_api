...
- **EF Core against SQL Server**, chosen over PostgreSQL (used in production, but not something I have direct experience defending) and SQLite (would have avoided the multi-container Docker setup, but SQL Server-in-a-container is the more useful thing to practise given it's also the more production-realistic setup for this kind of platform).
...
| `pageNumber` | int | Defaults to `1`. |
| `pageSize` | int | Defaults to `20`. No upper cap currently enforced (no `Validator.cs` for this slice). |
...
- Once that pattern was proven out and stable, a Claude Code skill (`/add-slice`) was created to scaffold new slices following the same structure — nested partial class, `Request`/`Response`/`Validator`/`Handler`/`Repository`, separate `{SubFeature}Controller`, FluentValidation wiring. This was used to generate the `SearchActors` slice, which closely mirrors `SearchMovies` in shape.
